...
function removeFirstOccurrences(str, value) {
  ...
  const a = str.slice(0, str.indexOf(value));
  const c = str.slice(str.indexOf(value) + b);
  return String.prototype.concat(a, c);
}
...
function removeLastOccurrences(str, value) {
  if (str.indexOf(value) === -1) return str;
  const b = value.length;
  const a = str.slice(0, str.lastIndexOf(value));
  const c = str.slice(str.lastIndexOf(value) + b);
  return String.prototype.concat(a, c);
}
...
function countVowels(str) {
  const v = str.match(/[aeiouyAEIOUY]/gi);
  if (v === null) return 0;
  return str === '' ? 0 : v.length;
}
...
function isPalindrome(str) {
  const str1 = str.toLowerCase().replace(/[, ?!]/gi, '');
...
function findLongestWord(sentence) {
  const longest = sentence.split(' ').sort((a, b) => {
...
function extractNameFromTemplate(value) {
  return value.replace(/(Hello, |!)/gi, '').trim();
}
